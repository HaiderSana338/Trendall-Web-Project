...
function InventorySearchBar({ searchData, type, vaseData, onSelect, onSearchAndClose }) {
    ...
    useEffect(() => {
        // Display all data when search input is empty
        showAllData();
    }, [searchData, vaseData]);
    ...
    const handleInputChange = (e) => {
        ...
        const matches = searchData
            ...
            .map(option => {
                const count = vaseData.filter(vase => vase.InventoryID === option ).length;
    ...
    const handleSelectName = (name) => {
        ...
        setMatchedItems([]);
        setShowSuggestions(false);
        onSelect(name); // Call the onSelect function passed as prop
    };
